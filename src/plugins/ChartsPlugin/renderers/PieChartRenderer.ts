...
export class PieChartRenderer extends BaseChartRenderer {
  public render(ctx: CanvasRenderingContext2D, data: ChartPoint[], options: ChartOptions): void {
    ...
    if (orientation === 'vertical') {
      this.drawVerticalSlices(ctx, validData, total, centerX, centerY, radius, options);
      this.drawVerticalLabels(ctx, validData, total, centerX, centerY, radius, options);
    } else {
      this.drawHorizontalSlices(ctx, validData, total, centerX, centerY, radius, options);
