...
```javascript
import { Editor, BlockStylePlugin } from 'on-codemerge';
...
Factory: `BlockStylePlugin()`.

No named commands — toolbar / menu UI only.

> **Note:** Toolbar UI only — no named commands / instance API.
...
<div class="custom-block">This block uses a CSS class</div>

<!-- Multiple classes -->
<p class="highlighted important">This paragraph has multiple classes</p>
```
...
function MyEditor() {
  ...
  useEffect(() => {
    if (editorRef.current && !editorInstance.current) {
      ...
      editorInstance.current.use(BlockStylePlugin());
    }
...
export default {
  ...
  mounted() {
    ...
    this./* use plugins: [BlockStylePlugin()] in Editor(...) */;
  },
  beforeDestroy() {
    if (this.editor) {
      this.editor.destroy();
