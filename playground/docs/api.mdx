...
To integrate the `I18n` class into the API documentation for **On-Codemerge**, we can add a new section that explains its purpose, usage, and methods. Here's how you might structure this addition:
...
The `I18n` class in On-Codemerge provides robust support for internationalization, allowing you to easily manage and implement multiple languages within the editor.


### Constructor

- **`I18n(core: EditorCore, locale: string = 'en')`**

Initializes a new instance of the internationalization class.

- `core`: The instance of `EditorCore`.
- `locale`: The default language code (e.g., 'en' for English).

### Methods

- **`addObserver(observer: Observer): void`**

Registers an observer that will be notified of language changes.

- **`notifyObservers(): void`**

Notifies all registered observers of the current language.
...
Merges new translations into the current language pack.
...
A key-value mapping of translation strings.

- **`Languages`**

A mapping of language codes to their respective `LanguagePack`.

- **`Observer`**

An interface for observers that need to react to language changes.
